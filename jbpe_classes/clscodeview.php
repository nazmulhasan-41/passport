...
function cmbCurrencyList($conn) {
	...
	$sql_currency = "select ISO, CURRENCY_CODE,CURRENCY_NAME FROM [dbjbPassportEndorse].[dbo].[tblCurrencyList] WHERE STATUS='A'";
	$result_currency=sqlsrv_query($conn,$sql_currency);
	
	$this->cmbCurrencyList.="<select onclick=OnSelectionChange(this.value) name=\"cmbCurrencyList\" id=\"cmbCurrencyList\">";

	$this->cmbCurrencyList.="<option value='' selected>--Select Currency--</option>";
	
	while($row = sqlsrv_fetch_array($result_currency,SQLSRV_FETCH_BOTH)){


			$this->getExchangeRate($row['ISO'],$conn);
			$rate =$this->exchangeRate;

		$this->cmbCurrencyList.="<option value=\"".$row['ISO'].' '.$rate."\">".$row['CURRENCY_NAME']."</option>";	
...
function cadExchangeRate($conn) {
	...
	$sql_country = "SELECT [Rate] FROM [dbjbPassportEndorse].[dbo].[ExchangeRate]
				...
				(	[Rate]
					...
					WHERE [ISO]='CAD' AND [SUBMISSION_STATUS]='A' AND [DATE]=CONVERT(date,GETDATE());";
	
	$result_cadExchangeRate=sqlsrv_query($conn,$sql_country);
	
	while($row = sqlsrv_fetch_array($result_cadExchangeRate,SQLSRV_FETCH_ASSOC)){
	
		$this->cadExchangeRate=$row['Rate'];	
	
	}

	sqlsrv_free_stmt($result_cadExchangeRate);	
}

function getExchangeRate($currencyCode, $conn) {
	...
	$sql_country = "SELECT [Rate] FROM [dbjbPassportEndorse].[dbo].[ExchangeRate]
				...
				(	[Rate]
					...
					WHERE [ISO]='$currencyCode' AND [SUBMISSION_STATUS]='A' AND [DATE]=CONVERT(date,GETDATE());";
	


	$result_exchangeRate=sqlsrv_query($conn,$sql_country);
	
	
	while($row = sqlsrv_fetch_array($result_exchangeRate,SQLSRV_FETCH_ASSOC)){
		$this->exchangeRate = $row['Rate'];	

		//echo '===>>'.$row['Rate'];	
	}
	sqlsrv_free_stmt($result_exchangeRate);	
}
