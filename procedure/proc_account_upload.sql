...
-- =============================================
ALTER PROCEDURE proc_account_upload 
@branch_code varchar(4),@soft_id int,@account_type VARCHAR(3),@upload_file_name VARCHAR(50),@temp_upload_file_name VARCHAR(50),@current_user VARCHAR(10),@user_ip VARCHAR(20)
...
--DECLARE @systemstatus CHAR(1),@proc_key char(1),@currdate DATETIME,@branchcode varchar(4),@existkey VARCHAR(10),@adviceno INT,@randomnumber INT,@newkey VARCHAR(10);
	...
	BEGIN TRY
		IF @trancount = 0
			BEGIN TRANSACTION
		ELSE
			SAVE TRANSACTION proc_account_upload;
--===========================
...
(
FIRSTROW = 0,
FORMATFILE = '+ CHAR(39) + 'C:\sanctions_download\account_upload_format.fmt' + CHAR(39) +',
...
--Check Branch Data
DECLARE @exit_branch VARCHAR(4);
select @exit_branch=branch_code from tbl_account_upload_temp where branch_code<>@branch_code

		IF @exit_branch<>@branch_code --System Down
		BEGIN
		SET @trancount = 1;
		SET  @xstate = 1;
		SET @message = 'Diffrent Branch Data File';
		RAISERROR ('proc_account_upload: %d: %s', 2, 1, 1, @message);
		GOTO lbexit
		END
...
select @slupload,@account_type,account_type_sbs,account_number,account_entity,owner_type,account_name,father_name,mother_name,spouse_name,birth_date,national_id,passport_no,present_address,permanent_address,telephone_no,mobile_no,email_address,branch_code,@current_user,file_date,country_code,'N' from tbl_account_upload_temp	
--INSERT TO BACK TABLE FOR UPDATE       tbl_account_upload_back
...
select @slupload,@branch_code,@soft_id,@account_type,GETDATE(),@upload_file_name,@totaldata,'P','',@current_user,@user_ip,GETDATE(),'N'
...
lbexit:
	...
	BEGIN CATCH
		...
		IF @xstate = 1 and @trancount = 0
			ROLLBACK
		IF @xstate = 1 and @trancount > 0
			ROLLBACK TRANSACTION proc_account_upload;

		RAISERROR ('proc_account_upload: %d: %s', 2, 1, @error, @message);
	END CATCH	
