...
function form_list_print(brcode,frm_date,to_date){
	//alert(brcode);
	if(brcode=='selected'){
	...
	var rurl='rpt_neps_enrolled_List_report_fpdf.php/' +brcode+ '/' +frm_date+ '/' +to_date;
	//alert(rurl);
	window.open(rurl);		
}
...
<link href="<?PHP echo $baseroot;?>/jbneps_lib/jrstl.css" rel="stylesheet" type="text/css">
...
<center>
  <table width="100%"  border="0" class="indexbox">
    ...
    <tr>
      <td width="100%" colspan="2" class="tbg"><strong>NEPS ENROLLED CUSTOMER REPORT</strong></td>
    </tr>
    <tr>
      <td colspan="2">
<?PHP
    ...
    <tr>
      <td colspan="2"><form action="" method="post" enctype="multipart/form-data" name="frmupload">
        <table width="100%" border="1" align="center" cellpadding="10"  cellspacing="2" bordercolor="#CCCCCC">
          <tr>
            <td colspan="5"><div align="center"><strong>Reconcile Report</strong></div></td>
			</tr>
			<tr>
            ...
            <td width="30%">
				<input name="txtfrm_date" type="text" id="txtfrm_date" value="<?PHP echo $frm_date;?>" size="15"  maxlength="10" placeholder="DD-MM-YYYY?" />
            ...
            <td width="30%">
				<input type="text" id="txtto_date"  value="<?PHP echo $to_date;?>" maxlength="10" size="15" name="txtto_date" placeholder="DD-MM-YYYY?" />
        ...
        <!--  <input type="button" name="Submit2" value="Show" onclick="form_list_print(cmbthisbranchlist.value,txtfrm_date.value,txtto_date.value);" />'<?php //echo $accRoutingCode;?>'),
         -->  <input type="button" name="Submit2" value="Show" onclick="reconcile('showCustomer',txtfrm_date.value,txtto_date.value,cmbthisbranchlist.value,'<?php echo $baseroot;?>');" />
