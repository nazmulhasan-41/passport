...
{ 
	...
	$this->Cell(18,5,'Passport No',1,0,'L');
	/* IF ($GLOBALS['branchName']=="ALL")
		{
			$this->Cell(9,5,'Br.Co.',1,0,'L');
		} */
	$this->Cell(45,5,'Applicant Name',1,0,'L');
	$this->Cell(19,5,'Contact No',1,0,'L');
	$this->Cell(109,5,'Address',1,0,'L');
	$this->Cell(16,5,'PsportDate',1,0,'L');
	$this->Cell(5,5,'TM',1,0,'C');
	$this->Cell(15,5,'EndrsDate',1,0,'C');
	$this->Cell(13,5,'FCAmnt',1,0,'R');
	$this->Cell(19,5,'Amnt. BDT',1,0,'R');
...
while ($row_form=sqlsrv_fetch_array($result_form2)) {
		...
		$pdf->Cell(18,5,$row_form['PASSPORT_NO'],1,0,'L');	
		/* IF ($GLOBALS['branchName']=="ALL")
				{
				  $pdf->Cell(9,5,$row_form['branchCode'],1,0,'L');
				}*/	 	
		$pdf->Cell(45,5,$row_form['CITIZEN_NAME'],1,0,'L');
		$pdf->Cell(19,5,$row_form['CONTACT_NO'],1,0,'L');
		$pdf->SetFont('Arial','',7);
		$pdf->Cell(109,5,$row_form['ADDRESS'],1,0,'L');
	    $pdf->SetFont('Arial','',7);
		$pdf->Cell(16,5,$row_form['PASSPORT_DATE'],1,0,'L');		
		$pdf->Cell(5,5,'T',1,0,'L');	
		$pdf->Cell(15,5,$row_form['TM_DATE'],1,0,'C');
		$pdf->Cell(13,5,$row_form['SFC_BANK'],1,0,'R');	
		$pdf->Cell(19,5,$row_form['AMOUNT_IN_BDT'],1,0,'R');
		...
		$pdf->Cell(13,5,''.number_format($TotalFC,2),1,0,'R');
		$pdf->Cell(19,5,''.number_format($TotalBdt,4),1,0,'R');
