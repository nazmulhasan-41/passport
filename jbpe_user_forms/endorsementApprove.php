				  ...
				  -->
				    <td >
							...
							if (empty($exchangeRate)){
							  ...
							  //$colHeader="Cust First Name,Cust Last Name,Birth Date,NID Number,Father Name,Mother Name,Acc Number,Acc Name,Routing Number,Mobile No,Bhata Type,Reject,Authorize";
							  //,Edit,Authorize
							  $colHeader="Passport No,Applicant Name,Address,PP Issue Date,PP Issue Place,Contact No,TM For,Amount Fr Curr.,Amount BDT.,Purpose,Company,Category,Reject,Authorize";
							/*  $sql="SELECT   [custFirstName]
										...
										WHERE [branchCode]=".$branchCode."AND [submissionStatus] = 'P'";
								*/
							$sql="SELECT pp.[PASSPORT_NO]
								...
								where tm.BRANCH_CODE= '".$branchCode."' AND tm.TRAN_STATE='P'
								AND tm.[TM_DATE]=convert(date, getdate())" ;
								
							   $dataviewclass->form_data_list_auth($tblcaption,$colHeader,$sql,$editlink,$baseroot,$conn);
