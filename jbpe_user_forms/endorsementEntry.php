	...
	with (form)
	  {
	  	...
	  	//alert ('checking');
			...
			else if (date_regex.test(passportDate.value)==false)
			{	alert('Date('+passportDate.value+') Pattern(DD-MM-YYYY) Does Not Match');
				passportDate.focus();return false;
			}
...
function handleClick(value)
{
	...
	if(value=='ck')
	{
		...
		document.getElementById('txt_sfc_bank').value="";
		document.getElementById('txt_total_bdt_amount').value="";
		document.getElementById('txt_total_fc_amount').value="";
		document.getElementById('txt_sfc_total').value="";
		document.getElementById('txt_amnt_input').value="";
		
    //  document.getElementById(txtcheque).value=chequeno;
		
	}
	else if(value=='ck_others'){
		// alert('in else if');
		console.log('clickeddd');
		document.getElementById('txt_amnt_input_others').style.display = 'block';
	...
	else
	{
		...
		document.getElementById('txt_total_bdt_amount').value=(value*exchangeRateIndia).toFixed(4);
...
function handleClick2(value)
{
	...
	if(value=='ck')
	{
		...
		document.getElementById('txt_amnt_input_forAll').style.display = 'block';
		document.getElementById('txt_sfc_bank_forAll').value="";
		document.getElementById('txt_total_bdt_amount_forAll').value="";
		document.getElementById('txt_total_fc_amount_forAll').value="";
		document.getElementById('txt_sfc_total_forAll').value="";
     ...
     <tr>
	   ...
	   <td>
				<?php
						...
						// $exchangeRate =$codeviewclass->;
						// echo $exchangeRate;

						$codeviewclass->cadExchangeRate($conn);
						$cadexchangeRate =$codeviewclass->cadExchangeRate;
						// echo $cadexchangeRate;


						/// currency lists
						$codeviewclass->cmbCurrencyList($conn);
						$currencyLists =$codeviewclass->cmbCurrencyList;
						// echo $currencyLists;
						...
						<td >
								<!--<input name="txt_currency" type="text" id="txt_currency"  value="1" size="20" autocomplete="off"  placeholder="CURRENCY For?" readonly required /> -->
								 <select id="currency" name="currency">
									  <option value="001" selected>USD</option>
								</select>
				...
				<table id="tmForm">
					...
					<tr>
						<td>
								...
								<label class="w3-text-teal w3-text-black"><b>Select Currency</b></label>
						</td>
						<td>	

							<?php echo $currencyLists; ?>
					...
					<?php
						...
						// $codeviewclass->getExchangeRate('CAD',$conn);
						// $rate =$codeviewclass->exchangeRate;
					...
					<script>
												...
												const splittedArray = value.split(" ");
														...
														document.getElementById("check").innerHTML =splittedArray[0];
														document.getElementById("check2").innerHTML =splittedArray[1];

													}
											</script>

					<!-- end other currency selection  -->
					...
					<tr>
						...
						<td >
							   <label>
							   <input type="radio" name="txt_amnt" id="txt_amnt_150" value="150" onclick="handleClick2(this.value);"  <?PHP  if ($amount==150) {echo "checked";}?> /><?PHP // if ($amount==150) {echo $OnclickAmount;}?>
							   $150
							   </label>
							   
							   <label>
							   <input type="radio" name="txt_amnt" id="txt_amnt_200" value="200" onclick="handleClick2(this.value);" <?PHP if ($amount==200) {echo "checked";}?> /><?PHP // if ($amount==200) {echo $OnclickAmount;}?>
							   $200
							   </label>
							   
							   <label>
							   <input type="radio" name="txt_amnt" id="txt_amnt_ck" value="ck"  onclick="handleClick2(this.value);" <?PHP if (!empty($amount) AND !in_array($amount, $amountArr)) {echo "checked";}?> />
							   Other Amount
							    </label>

							   <input name="txt_amnt_input_forAll" type="number" id="txt_amnt_input_forAll" onkeyup="sfcAmount2(this.value)" value="<?PHP //echo $tm;?>" size="20" autocomplete="off"  placeholder="Amount ?" hidden required />
							  
								<span  class="error" >*1 
									<span id="check" ></span>

								   =<span class="error" id="check2" >
								   	
								   	<!-- <?PHP echo $exchangeRate;?> -->
					...
					<tr hidden>
						...
						<td>
								<input name="txt_total_fc_amount_forAll" type="text" id="txt_total_fc_amount_forAll"  value="<?PHP //echo $tm;?>" size="20" autocomplete="off"  placeholder="Total FC Amount?"  required />
						</td>
					</tr>
					...
					<tr>
						...
						<td>
								<input name="txt_total_bdt_amount_forAll" type="text" id="txt_total_bdt_amount_forAll"  value="<?PHP //echo $tm;?>" size="20" autocomplete="off"  placeholder="Total Amount In BDT?" readonly required />
						</td>
					</tr>
					...
					<tr hidden>	
						...
						<td>
								<input name="txt_sfc_bank_forAll" type="text" id="txt_sfc_bank_forAll"  value="150" size="20" autocomplete="off"  placeholder="SFC Bank" readonly required />
						</td>
					</tr>
					<tr hidden>	
						<td>
						...
						<td>
								<input name="txt_sfc_total_forAll" type="text" id="txt_sfc_total_forAll"  value="" size="20" autocomplete="off"  placeholder="SFC Total" readonly required />
						</td>
					</tr>
					<tr >	
						<td>
